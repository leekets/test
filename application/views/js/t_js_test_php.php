	...
	<?php }?>
$(function() {
	<?php include("section/v2/s_js_v2_basic.js"); ?>
	function act() {
...
//			sen1(comments[index]['a']);			//调用句库函数；
			...
			$("audio").attr({"src": ""});
			$("audio").attr({"src": "<?php echo base_url()?>audio/"+s_audio});
...
//			}else{
				...
				for(var i=0;i<errors.length;i++){
					if(errorsID==""){
						errorsID=errors[i].ID
					}else{
						errorsID=errorsID+','+errors[i].ID;
					}
					//alert(errors[i].ID);
				}
				window.location=jumpUrl+"&error="+errorsID;
